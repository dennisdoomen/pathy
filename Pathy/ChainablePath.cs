...
namespace Pathy
{
...
#endif
    {
        ...
        /// </summary>
        /// <remarks>
        /// For a UNC path (e.g. <c>\\server\share\folder</c>), walking up through <see cref="Parent"/> stops
        /// at the share (e.g. <c>\\server\share</c>) rather than at the server, since a UNC path has no
        /// meaningful path above its share root.
        /// </remarks>
        public ChainablePath Parent
        {
            get
            {
                string directory = DirectoryName;
                return directory.Length > 0 ? From(directory) : Empty;
            }
        }
        ...
        /// <summary>
        /// Gets a value indicating whether the current path is likely to be usable on the current operating system
        /// and file system, based on the characters that are known to be invalid for paths and file/directory names.
        /// </summary>
        /// <remarks>
        /// This is a best-effort check, not a guarantee. What counts as a valid path or file name differs per
        /// platform and per file system (and can even depend on host configuration, such as long path support),
        /// so a path for which <see cref="IsValid"/> returns <c>true</c> may still fail when it is actually used,
        /// and vice versa. The root of the path (such as a drive letter or a UNC server/share) is not checked,
        /// since those parts are not free-form user input and can legitimately contain characters (such as
        /// <c>:</c> or <c>\\</c>) that would be invalid elsewhere in the path.
        /// </remarks>
        public bool IsValid
        {
            get
            {
                ...
                {
                    if (segment.Length > 0 && segment.IndexOfAny(invalidNameChars) >= 0)
                    {
                        return false;
                    }
                }
    ...
    public ChainablePath AsRelativeTo(ChainablePath basePath)
    {
        ...
        /// <summary>
        /// Removes <c>.</c> segments and resolves <c>..</c> segments without reading the current directory or file system.
        /// </summary>
        /// <remarks>
        /// The result keeps the path relative or absolute. Leading <c>..</c> segments in a relative path are kept when
        /// there is no earlier segment to remove. A <c>..</c> segment at the root of an absolute path is ignored, so
        /// normalization never moves above the root.
        /// </remarks>
        public ChainablePath Normalize()
        {
            ...
            bool isRooted = root.Length > 0;
            bool keepTrailingSlash = EndsInDirectorySeparator(normalizedPath) && normalizedPath.Length > root.Length;
            var segments = new List<string>();

            foreach (string segment in normalizedPath.Substring(root.Length).Split(Path.DirectorySeparatorChar))
            {
            ...
            string normalized = root + string.Join(Path.DirectorySeparatorChar.ToString(), segments);

            if (keepTrailingSlash && normalized.Length > root.Length)
            {
                normalized += Path.DirectorySeparatorChar;
            }

            return normalized.Length > 0 ? new ChainablePath(normalized) : Empty;
        }
